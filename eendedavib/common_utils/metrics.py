...
def calculate_metrics(
    ...
    for seq_num in range(target.shape[0]):
        ...
        if(decisions.ndim == 3):
            d_seq = decisions[seq_num, :boundary, :]  # [B, F, D]
        ...
        elif(decisions.ndim == 4): #with sample
            raise NotImplmentedError
            d_seq = decisions[:, seq_num, :boundary, :]  # [M, B, F, D]
            pred_spk_qty = torch.round(d_seq.sum(axis=2).mean(axis=0))
